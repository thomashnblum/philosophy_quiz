...
function determineBranch(v: number[]) {
  const scores = {
    A: -v[0] + -v[1], // Metafísica/Epistemologia negativa
    B: v[1] + -v[4], // Epistemologia positiva / Linguagem negativa
    C: -v[2] + v[3], // Ética negativa / Política positiva
    D: v[4] + v[5]   // Linguagem/Subjetividade positiva
  };
...
function selectRefinement(top5ids: number[]) {
  ...
  for (const [k, ids] of Object.entries(REFINE_CLUSTERS)) {
    const overlap = top5ids.filter(id => ids.includes(id));
    if (overlap.length >= 2 && sel.length < 2) sel.push(k);
  }
  if (sel.length === 0) sel.push('QR4', 'QR5');
  else if (sel.length === 1) sel.push('QR5');
...
export default function App() {
  ...
  const handleNext = () => {
    ...
    const newVector = vector.map((v, i) => v + opt.v[i]);
    setVector(newVector);
    setSelectedOption(null);
    ...
    if (!isLast) {
      if (phase === 0 && currentIndex === 4) {
        const branch = determineBranch(newVector);
        const bqs = BRANCH_QUESTIONS[branch] || [];
        setQueue(prev => [...prev, ...bqs]);
        setPhase(1);
      }
      setCurrentIndex(prev => prev + 1);
    } else {
      if (phase === 1) {
        const norm = newVector.map(v => v / (currentIndex + 1));
        const maxDist = Math.sqrt(7 * 36);
        const top5 = Object.values(SCHOOLS)
          .map(s => ({ ...s, aff: Math.max(0, 100 - (euclidean(norm, s.vector) / maxDist) * 100) }))
        ...
        setScreen('thinking');
        setTimeout(() => {
          const finalResult = computeResult(newVector, currentIndex + 1);
  ...
  const computeResult = (v: number[], count: number) => {
    const norm = v.map(val => val / count);
    const maxDist = Math.sqrt(7 * 36);
    ...
    return {
      ...
      all: ranked,
      norm,
      radarData: norm.map(val => ((val + 3) / 6) * 100),
      edgeCase,
      edgeMsg,
        ...
        <AnimatePresence mode="wait">
          ...
          {screen === 'home' && (
            <motion.section 
              ...
              exit={{ opacity: 0, y: -20 }}
              className="min-h-[calc(100vh-4.5rem)] flex flex-col items-center justify-center p-6 text-center max-w-4xl mx-auto"
            >
              <span className="bg-black text-white px-3 py-1 text-[10px] font-bold uppercase tracking-widest mb-6">Arquétipos do Pensamento</span>
              <h1 className="text-5xl md:text-8xl font-black leading-none tracking-tighter mb-8 uppercase">
                Onde sua alma reside?
              </h1>
              <p className="text-muted text-xl font-bold max-w-2xl mx-auto leading-tight mb-12">
              ...
              <div className="flex flex-col items-center gap-6">
                ...
                <div className="bg-white border-2 border-black p-4 flex gap-8">
                  ...
                  <div className="flex flex-col items-start">
                    ...
                    <span className="font-mono font-bold">48 TOTAL</span>
                  </div>
                </div>
              </div>
            </motion.section>
          ...
          {screen === 'quiz' && currentQuestion && (
            <motion.section 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="min-h-[calc(100vh-4.5rem)] flex flex-col p-6"
            >
              <div className="w-full max-w-5xl mx-auto grid md:grid-cols-12 gap-12 mt-12">
                <div className="md:col-span-8">
                  <div className="bg-acid-pink text-white px-3 py-1 text-[10px] font-bold uppercase tracking-[0.35em] mb-3 inline-block">
                    {currentQuestion.cat}
                  </div>
                  <h2 className="text-4xl md:text-6xl font-black leading-none mb-12 uppercase tracking-tighter">
                  ...
                  <div className="grid gap-4 mb-12">
                    {currentQuestion.opts.map((opt, i) => (
                      <div key={i} className="group cursor-pointer" onClick={() => handleSelect(i)}>
                        <div className={`
                          flex items-center gap-6 p-6 border-4 border-black transition-all duration-150
                          ${selectedOption === i ? 'bg-black text-white neo-shadow-sm translate-x-0.5 translate-y-0.5' : 'bg-white text-black neo-shadow hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]'}
                        `}>
                          <span className={`text-2xl font-black ${selectedOption === i ? 'text-acid-green' : 'text-black'}`}>
                            {opt.l}
                          </span>
                          <p className="text-xl font-bold leading-tight">{opt.t}</p>
                          {selectedOption === i && <div className="ml-auto w-6 h-6 bg-acid-green rounded-full shadow-[2px_2px_0px_0px_rgba(255,255,255,1)]"></div>}
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    <button 
                      ...
                      className={`flex items-center gap-2 font-black uppercase text-sm border-b-4 border-black transition-opacity ${currentIndex === 0 ? 'opacity-0 pointer-events-none' : 'opacity-30 hover:opacity-100'}`}
                    >
                      ← Voltar
                    </button>
                    <button
                      ...
                      onClick={handleNext}
                      className={`px-12 py-4 bg-black text-white border-4 border-black font-black uppercase shadow-[6px_6px_0px_0px_rgba(0,255,65,1)] transition-all ${selectedOption === null ? 'opacity-30 cursor-not-allowed' : 'hover:-translate-x-1 hover:-translate-y-1 hover:shadow-[10px_10px_0px_0px_rgba(0,255,65,1)]'}`}
                    >
                ...
                <aside className="hidden md:block md:col-span-4 h-fit border-4 border-black bg-gold p-8 neo-shadow">
                  <div className="flex flex-col items-center justify-center text-black">
                    <span className="text-8xl font-black leading-none">{(currentIndex + 1).toString().padStart(2, '0')}</span>
                    <span className="text-[12px] font-bold uppercase mt-2 tracking-widest">QUESTÃO {currentIndex + 1} DE {queue.length}</span>
                  </div>
                  <div className="mt-8 pt-8 border-t-2 border-black/20">
                    <p className="text-xs font-bold leading-tight uppercase opacity-60 mb-2">Sistema Status</p>
          ...
          {screen === 'thinking' && (
            <motion.section 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="min-h-[calc(100vh-4.5rem)] flex flex-col items-center justify-center p-6 text-center"
            >
              <div className="w-16 h-16 border-8 border-black border-t-acid-green animate-spin mb-8" />
              <p className="text-4xl font-black uppercase tracking-tighter">Sintetizando sua ontologia...</p>
            </motion.section>
          )}
          ...
          {screen === 'result' && result && (
            <motion.section 
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="min-h-screen p-6 md:p-12 mb-10"
            >
              <div className="max-w-7xl mx-auto">
                <div className="mb-16 border-4 border-black bg-white p-12 neo-shadow flex flex-col md:flex-row justify-between items-center gap-8">
                  <div>
                    ...
                    <h1 className="text-6xl md:text-8xl font-black uppercase tracking-tighter leading-none mb-4">{result.primary.name}</h1>
                    <p className="font-mono font-bold text-xl uppercase opacity-60">{result.primary.era} // {result.primary.thinkers}</p>
                  </div>
                  <div className="bg-black text-acid-green p-8 border-4 border-black neo-shadow-sm flex flex-col items-center justify-center min-w-[200px]">
                    <span className="text-5xl font-black leading-none">{result.primary.pct}%</span>
                    <span className="text-[10px] font-bold uppercase tracking-widest mt-2">Compatibilidade</span>
                  </div>
                </div>

                {result.edgeCase && (
                  <div className="bg-gold border-4 border-black p-8 mb-16 neo-shadow flex items-start gap-6">
                    <div className="w-12 h-12 bg-black text-white shrink-0 flex items-center justify-center font-black text-2xl group">
                      <span className="animate-pulse">!</span>
                    </div>
                    <p className="font-bold text-lg leading-snug uppercase">{result.edgeMsg}</p>
                  </div>
                )}

                <div className="grid lg:grid-cols-12 gap-12 mb-20 items-stretch">
                  <div className="lg:col-span-4 flex flex-col gap-12">
                    <section className="bg-white border-4 border-black p-8 neo-shadow flex-1">
                      <h3 className="font-black uppercase text-xl mb-8 border-b-4 border-black pb-2">Status dos Eixos</h3>
                      <div className="grid gap-6">
                        {AXES.map((label, i) => {
                          const val = Math.round(((result.norm[i] + 3) / 6) * 100);
                          return (
                            <div key={i} className="flex flex-col gap-2">
                              <div className="flex justify-between font-mono font-bold text-xs uppercase">
                                <span>{label}</span>
                                <span>{val}%</span>
                              </div>
                              <div className="h-6 border-2 border-black bg-bg overflow-hidden p-0.5">
                                <motion.div 
                    ...
                    <section className="bg-acid-green border-4 border-black p-8 neo-shadow">
                      <h3 className="font-black uppercase text-xl mb-8 border-b-4 border-black pb-2 text-black">Afinidades</h3>
                      <div className="grid gap-6">
                        {[result.secondary, result.tertiary, ...result.all.slice(3, 5)].map((s, i) => (
                          <div key={i} className="flex justify-between items-center group cursor-pointer" onClick={() => setSelectedSchool(s)}>
                            <div>
                              <div className="font-black text-xl uppercase tracking-tighter transition-colors group-hover:text-white">{s.name}</div>
                              <div className="text-[10px] font-bold uppercase opacity-60">{s.era}</div>
                            </div>
                            <div className="font-mono font-black text-lg">{s.pct || Math.round(s.aff)}%</div>
                          </div>
                        ))}
                      </div>
                    </section>
                  ...
                  <div className="lg:col-span-8 flex flex-col gap-12">
                    <div className="bg-white border-4 border-black p-8 md:p-12 neo-shadow flex flex-col items-center justify-center">
                          ...
                          options={{
                            scales: {
                              r: {
                                ...
                                angleLines: { color: 'rgba(0, 0, 0, 0.1)', lineWidth: 2 },
                                pointLabels: { 
                                  color: '#000000', 
                      ...
                      <div className="text-left w-full">
                        <div className="space-y-8 mb-12">
                          {result.primary.desc.map((p: string, i: number) => (
                            <p key={i} className="text-2xl font-black leading-tight uppercase tracking-tighter">{p}</p>
                          ))}
                        </div>
                        
                        <div className="bg-black text-white p-8 neo-shadow-sm mb-8">
                          <span className="block text-[10px] font-bold uppercase tracking-[0.3em] text-acid-green mb-4">Tensões de Pensamento</span>
                          <p className="text-xl font-bold uppercase leading-none">{result.primary.tensions}</p>
                        </div>

                        <div className="border-l-8 border-black pl-8 py-4 mb-12 bg-acid-pink/10">
                          <q className="text-2xl font-black uppercase tracking-tighter block mb-4 italic">"{result.primary.quote}"</q>
                          <cite className="block font-mono font-bold text-lg opacity-60 not-italic">// {result.primary.author}</cite>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {screen === 'explore' && (
            <motion.section 
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="min-h-screen p-6 md:p-12 mb-20"
            >
              <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12 border-b-8 border-black pb-8">
                  <div>
                    <span className="bg-black text-white px-3 py-1 text-[10px] font-bold uppercase tracking-[0.4em]">O Grande Repositório</span>
                    <h2 className="text-4xl md:text-8xl font-black uppercase mt-2 tracking-tighter">Sabedoria</h2>
                  </div>
                ...
                <div className="flex flex-wrap gap-4 mb-12">
                  <button 
                    ...
                    className={`px-6 py-2 text-[10px] font-bold uppercase tracking-widest border-4 transition-all ${eraFilter === 'Todas' ? 'bg-black border-black text-white' : 'border-black text-black hover:bg-gold'}`}
                  >
                    Todas
                  </button>
                  {Object.keys(ERAS).map(era => (
                    <button 
                      ...
                      className={`px-6 py-2 text-[10px] font-bold uppercase tracking-widest border-4 transition-all ${eraFilter === era ? 'bg-black border-black text-white' : 'border-black text-black hover:bg-gold'}`}
                    >
                      {era}
                    </button>
                  ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                  {filteredSchools.map(s => {
                    ...
                    const isResultAffinity = [result?.secondary?.id, result?.tertiary?.id].includes(s.id);
                    ...
                    return (
                      <div 
                        ...
                        className={`
                          p-8 border-4 border-black bg-white transition-all duration-150 cursor-pointer group flex flex-col
                      ...
                      >
                        ...
                        <h3 className="text-3xl font-black uppercase tracking-tighter leading-none mb-4 group-hover:text-acid-pink transition-colors">{s.name}</h3>
                        <p className="text-black text-sm font-bold leading-tight uppercase opacity-70 line-clamp-3 mb-8">{s.desc[0]}</p>
